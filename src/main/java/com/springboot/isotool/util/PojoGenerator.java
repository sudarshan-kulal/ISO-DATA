...
public class PojoGenerator {
	...
	CannotCompileException{
		...
		System.out.println(cc.getClassPool());
		System.out.println(pool.insertClassPath("C:\\Sudarshan\\Projects\\ISO_TOOL\\ISO-DATA"));
		...
		try {
			for (Entry<String, Class<?>> entry : properties.entrySet()) {
				...
				cc.addField(new CtField(resolveCtClass((Class<Serializable>) entry.getValue()), entry.getKey(), cc));
	...
	private static CtClass resolveCtClass(Class<Serializable> clazz) throws NotFoundException {
	    ClassPool pool = ClassPool.getDefault();
